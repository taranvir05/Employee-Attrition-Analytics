...
st.markdown(
    ...
    <div class="saas-card" style="padding: 36px 40px; margin-bottom: 24px; background: linear-gradient(135deg, rgba(15, 23, 42, 0.85) 0%, rgba(30, 41, 59, 0.75) 100%); border: 1px solid rgba(56, 189, 248, 0.25);">
        <div style="display: flex; gap: 10px; margin-bottom: 16px; align-items: center;">
            ...
            <span class="badge-pill badge-success">v2.5 Active</span>
        </div>
...
with col_hero_btn1:
    if st.button("Predict Employee Attrition", key="btn_hero_predict"):
        st.switch_page("pages/2_Predict_Attrition.py")

with col_hero_btn2:
    if st.button("Explore HR Dashboard", key="btn_hero_dashboard"):
...
with col_hero_pills:
    ...
    st.markdown(
        ...
        <div style="display: flex; gap: 8px; justify-content: flex-end; align-items: center; height: 100%; flex-wrap: wrap;">
            ...
            <span class="badge-pill badge-purple" style="font-size: 0.82rem; padding: 6px 14px;">{_feature_count} Features</span>
...
# ==========================================
render_section_header("Company Overview", "Key workforce metrics & turnover health indicators", "KPI DASHBOARD", icon_name="activity")

k1, k2, k3, k4, k5, k6 = st.columns(6)

with k1:
    render_kpi_card("Employees", f"{total_emp:,}", "Total Headcount", "user", "Dataset", "cyan", [35, 50, 65, 80, 85, 92])
with k2:
    render_kpi_card("Attrition Rate", f"{attrition_rate:.1f}%", "Overall Turnover", "trending-up", "Turnover", "danger", [90, 75, 60, 45, 30, 20])
with k3:
    render_kpi_card("Average Salary", f"${avg_salary:,.0f}", "Monthly Base", "activity", "Compensation", "success", [40, 55, 60, 75, 85, 95])
with k4:
    render_kpi_card("Departments", f"{depts}", "Active Units", "home", "Divisions", "purple", [70, 70, 70, 70, 70, 70])
with k5:
    render_kpi_card("Average Age", f"{avg_age:.1f} yrs", "Demographics", "user", "Age Profile", "cyan", [50, 52, 51, 53, 52, 54])
with k6:
    render_kpi_card("Retention Score", f"{retention_score:.1f}%", "Stability Score", "shield-check", "Retention", "success", [60, 70, 75, 80, 82, 88])
...
# ==========================================
render_section_header("Workforce Analytics Widgets", "Interactive Plotly visual breakdown of turnover patterns", "ANALYTICS GRID", icon_name="bar-chart")
...
# ==========================================
render_section_header("Executive Business Insights", "Synthesized findings for corporate decision-makers", "INSIGHT CARDS", icon_name="lightbulb")
...
# ==========================================
render_section_header("Quick Platform Actions", "Launch key HR intelligence tools instantly", "QUICK ACTIONS", icon_name="zap")
